...
// Routes For Student
// get all students
routeForStudent.route("/").get(async (req, res) => {
  if (req.query.isMentored === "true") {
    let studentData = await client
      .db("mentorstudent")
      .collection("students")
      .find({ mentorstudent: true })
      .toArray();

    res.send({
      result: true,
      category: "only assigned students",
      data: studentData,
    ...
    res.send({
      result: true,
      category: "only unassigned students",
      data: studentData,
      totalStudents: studentData.length,
    });
  } else {
...
routeForStudent.route("/unassign").get(async (req, res) => {
  let studentData = await client
    .db("mentorstudent")
    .collection("students")
    .find({ mentorstudent: false })
...
routeForStudent
  ...
  .get(async (req, res) => {
    ...
    studentData.length === 0
      ...
      : res.send({ result: true, data: studentData });
  })
  ...
  .put(async (req, res) => {
    ...
    if (!studentData.mentorstudent) {
      let assignMentor = await client
        .db("mentorstudent")
        .collection("students")
        .updateOne(
          { studentName: req.params.name },
          { $set: { mentorDetails: { ...mentorData }, mentorstudent: true } }
        );
      ...
      let assignStudent = await client
        ...
        .updateOne(
          ...
          { $set: { students: stuForMen, numOfStudents: stuForMen.length } }
        );
      ...
      res.send({
        result: true,
        response: { assignMentor, assignStudent },
        data: student,
      });
    } else {
...
routeForStudent.route("/create").post(async (req, res) => {
  ...
  let checkStudent = students.filter(
    (student) => student.studentName === studentValidName
  );
  ...
  if (checkStudent) {
    let dbResponse = await client
      ...
      .insertOne({
        ...req.body,
        studentId: `220${students.length + 1}`,
        studentName: studentValidName,
        mentorstudent: !req.body.mentorDetails ? false : true,
      });
...
routeForMentor.route("/").get(async (req, res) => {
  let query = req.query;
  console.log(query);
  if (query.housefull === "true") {
    ...
    res.status(200).send({
      result: true,
      category: "only housefulled mentors",
      data: mentorData,
...
routeForMentor.route("/students/:mentorname").get(async (req, res) => {
  ...
  studentData.length === 0
    ? res.status(400).send({ message: "invalid" })
    : res.send({ result: true, data: studentData[0].students });
});

//  create mentor
routeForMentor.route("/create").post(async (req, res) => {
  ...
  let checkMentor = mentors.filter(
    (mentor) => mentor.mentorName === mentorValidName
  );
  ...
  if (checkMentor) {
    let dbResponse = await client
      ...
      .insertOne({
        ...req.body,
        mentorId: `10220${mentors.length + 1}`,
        mentorName: mentorValidName,
        numOfStudents: req.body.students.length,
        houseFull: req.body.students.length >= req.body.maxOfMen ? true : false,
      });
